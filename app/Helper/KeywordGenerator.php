...
class KeywordGenerator {
   ...
   */
  public $ThrWordsMinimumLength = 0;
  ...
  public function generateKW($str) {
    ...
    if ($wordsOne !== false) {
      ...
      array_push($keywordsArr, $wordsOne);
    }
    #----
    if ($wordsTwo !== false) {
      $twow_kw = implode(',', $wordsTwo) .',';
        array_push($keywordsArr, $wordsTwo);
    }
    #----
    if ($wordsThr !== false) {
      $thrw_kw = implode(',', $wordsThr) .',';
        array_push($keywordsArr, $wordsThr);
    }
    #----
  ...
  private function StripIgnoredWords($str) {
    ...
    */
    $lang = $this->lang == 'en' ? '2':'1';
    $ignore = Ignore::where('lang',$lang)->get();
